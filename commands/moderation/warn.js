...
module.exports.run = async (client, message, args) => {
  ...
  if(!reason) {
    return message.channel.send( "Please enter a reason to warn! \n" + `**USAGE:** \`${prefix ? prefix : client.prefix["default"]}warn <@mention/id> <reason>\``)
  }
  ...
  if(warnings === 5) {
    return message.channel.send(`**ERROR**! ${message.mentions.user.first().username} has already reached the maximum warns.`)
  }
  if(warnings === null) {
    db.set(`warnings_${message.guild.id}_${member.id}`, 1, message.author.id)
    member.send("**WARN** You have been warned  in " + message.guild.name + " for " + '`' + reason + '`' + ". \n You have been warned by " + message.author.tag)
    await message.channel.send("**SUCCESS!** You have warned " + "<@" + member.id  + ">"  + ".")
